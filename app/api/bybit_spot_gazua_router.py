...
router = APIRouter(prefix="/api/strategy/bybit_spot_gazua", tags=["BYBIT_SPOT_GAZUA"])
...
def _get_um(request: Request):
    ...
    if um is None:
        ...
        um = BybitSpotGazuaManager(system=system)
        system.bybit_spot_gazua_manager = um
    return um


# ── Status / Config ─────────────────────────────────────────
@router.get("/status")
def bybit_spot_focus_status(request: Request):
    return {"ok": True, **_get_um(request).get_status()}


@router.get("/config")
def bybit_spot_focus_config_get(request: Request):
...
@router.post("/config")
def bybit_spot_focus_config_set(
    request: Request,
    ...
    logger.info("[BYBIT_SPOT_FOCUS_API] config set: %d fields", len(cfg))
...
def bybit_spot_focus_enable(
    ...
    if budget is not None:
        cfg["budget"] = budget
    um.update_config(cfg)
    logger.info("[BYBIT_SPOT_FOCUS_API] Enabled: %s", {k: v for k, v in cfg.items() if k != "enabled"} or "none")
    return {"ok": True, "enabled": True, **um.get_status()}


@router.post("/disable")
def bybit_spot_focus_disable(request: Request):
    um = _get_um(request)
    um.update_config({"enabled": False})
    logger.info("[BYBIT_SPOT_FOCUS_API] Disabled")
...
def bybit_spot_focus_scan(request: Request):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] scan error: %s", exc)
...
def bybit_spot_focus_scan_candidates(request: Request):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] scan-candidates error: %s", exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_scan_list(request: Request):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] scan-list error: %s", exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_score_timeline(request: Request, market: str = Query(..., description="마켓 (예: WLDUSDT)"),
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] score-timeline %s error: %s", market, exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_orderbook(request: Request, market: str = Query(..., description="마켓 (예: BTCUSDT)"),
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] orderbook %s error: %s", market, exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_journal(request: Request, limit: int = Query(100, ge=1, le=2000)):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] journal error: %s", exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_journal_delete(request: Request, ts: float = Query(..., description="삭제할 저널 기록의 ts (행 고유값)")):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] journal delete error: %s", exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_order(
    ...
    try:
        ...
        logger.info("[BYBIT_SPOT_FOCUS_API] quick_order %s %s -> %s", market, side, res.get("ok"))
        return res
    except Exception as exc:
        logger.warning("[BYBIT_SPOT_FOCUS_API] order %s %s error: %s", market, side, exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_force_close(request: Request, market: str = Query(..., description="강제청산할 봇 관리 포지션 마켓 (예: BTCUSDT)")):
    ...
    try:
        ...
        logger.info("[BYBIT_SPOT_FOCUS_API] force_close %s -> %s", market, res.get("ok"))
        return res
    except Exception as exc:
        logger.warning("[BYBIT_SPOT_FOCUS_API] force_close %s error: %s", market, exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_orphans(request: Request):
    ...
    try:
        ...
        logger.warning("[BYBIT_SPOT_FOCUS_API] orphans error: %s", exc)
        return {"ok": False, "error": str(exc)}
...
def bybit_spot_focus_adopt(request: Request, market: str = Query(..., description="사면할 코인 마켓 (예: WLDUSDT)")):
    ...
    try:
        ...
        logger.info("[BYBIT_SPOT_FOCUS_API] 🕊️ adopt %s -> %s", market, res.get("ok"))
        return res
    except Exception as exc:
        logger.warning("[BYBIT_SPOT_FOCUS_API] adopt %s error: %s", market, exc)
        return {"ok": False, "error": str(exc)}
