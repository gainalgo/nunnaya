...
# ────────────────────────────────────────────────────────────
# ★ [2026-05-19 owner decision] Automatic config snapshot system
# Owner's vision: "Keep a history of configs so we can roll back / compare."
# Now unified in app/core/config_history.py (all engines, hash-dedup, N + days
# retention). Snapshots are written automatically by the _save_config hook;
# the endpoints below list / fetch / restore them. See config_history for retention.
# ────────────────────────────────────────────────────────────
...
def focus_config_snapshots():
    """List saved config snapshots, newest first: [{name, ts, reason, mtime}, ...]."""
    from app.core import config_history
    return {"ok": True, "snapshots": config_history.list_snapshots("focus_config")}


@router.get("/config/snapshots/{filename}")
def focus_config_snapshot_single(filename: str):
    """Return a single snapshot's config block."""
    from app.core import config_history
    cfg = config_history.load_snapshot("focus_config", filename)
    if cfg is None:
...
def focus_config_snapshot_restore(
    request: Request,
    name: str = Query(..., description="snapshot filename to restore"),
):
    """Restore a saved snapshot via the validated update path (the restored config
    is itself snapshotted, so a restore can be undone)."""
    from app.core import config_history
    cfg = config_history.load_snapshot("focus_config", name)
    if cfg is None:
        return {"ok": False, "error": "snapshot not found"}
    result = _get_fm(request).update_config(cfg)
    return {"ok": True, "restored": name, "config": result}
...
def focus_set_config(
    ...
    # Config snapshot is recorded automatically by the config_history hook inside
    # _save_config (unified across all engines), so no explicit snapshot call here.
