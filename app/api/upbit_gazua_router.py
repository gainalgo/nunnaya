...
# ------------------------------------------------------------
# SpotGazuaManager 제어용 REST 엔드포인트. strategy_focus_router 미러.
# ============================================================
...
def _get_um(request: Request):
    """Get UpbitGazuaManager from system (없으면 생성)."""
...
def upbit_focus_config_set(
    ...
    # ★ generic: 위에 명시 안 된 672 미러 필드도 query param 으로 수용.
    #   update_config 가 dataclass 타입 강제변환 + hasattr 필터 → 안전(없는 키 무시).
