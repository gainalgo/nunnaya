 ...
 * 
 * TODO: Currency Abstraction
 * - Load quote currency config from /api/system/currency on init
 * - Replace hardcoded currency, "USDT", "$" with formatCurrency()
 * - API endpoint query params like budget_usdt should use quote currency symbol
 * ============================================================ */
...
const authFetch = (window.AutocoinUtils || {}).authFetch || ((url, opts) => fetch(url, { credentials: 'same-origin', ...opts }));
...
async function loadQuoteCurrencyConfig() {
  try {
    const resp = await authFetch('/api/ui/config');
    if (resp.ok) {
...
const API = {
  ...
  reservedRefresh: (ppN, alN, ldN, ltN, gzN, ctN, snN) => {
    const qs = new URLSearchParams();
...
const POLL_MS = 10000;  // 10초 (기존 3초 → 서버 부하 감소)
...
const state = {
  ...
  // 바이낸스 가격 (김프 계산용)
  binancePrices: {},  // { "BTC": 96500, "ETH": 3200, ... } USDT 가격
  exchangeRate: 1450,   // Exchange rate
  binanceLastFetch: 0,
};
...
async function loadMarketUpside() {
  try {
    ...
    if (!data.ok || !data.rankings || data.rankings.length === 0) {
      ...
      if (moreEl) moreEl.innerHTML = '<div class="empty" style="color:#666;">없음</div>';
      return;
    }
...
async function fetchBinancePrices(forceRefresh = false) {
  ...
  try {
    const res = await authFetch("/api/system/cross-compare?markets=");
    const data = await res.json();
...
function getKimchiPremium(market) {
  // market: "BTCUSDT" format
  const coin = market.includes("-") ? market.split("-")[1] : market;
  ...
  const binanceUsdt2 = binanceUsdt * rate;
  const premiumPct = ((bybitPrice - binanceUsdt2) / binanceUsdt2) * 100;
...
// 김프 표시 HTML 생성 (바이낸스 가격을 원화로 환산하여 표시)
function renderKimchiBadge(market, compact = false) {
  const kimchi = getKimchiPremium(market);
  if (!kimchi) return "";
  
  const pct = kimchi.premiumPct;
  const binanceUsdt2 = kimchi.binanceUsdt2;
  const color = pct > 0 ? "#f0b90b" : (pct < 0 ? "#4caf50" : "#888");  // 바이낸스 노란색
  
  // 바이낸스 원화 환산 가격 포맷
  const binanceUsdt2Fmt = binanceUsdt2 >= 1000000 
    ? (binanceUsdt2 / 1000000).toFixed(2) + "M" 
    : binanceUsdt2 >= 1000 
      ? Math.round(binanceUsdt2).toLocaleString()
      : binanceUsdt2.toFixed(1);
  
  const sign = pct > 0 ? "+" : "";
  const tooltip = `바이낸스: $${kimchi.binanceUsdt.toLocaleString()} × ${state.exchangeRate || 1450} USDT = $${Math.round(binanceUsdt2).toLocaleString()} (김프 ${sign}${pct}%)`;
  
  if (compact) {
    return `<span style="color:${color}; font-size:9px; margin-left:3px;" title="${tooltip}">B$${binanceUsdt2Fmt}</span>`;
  }
  
  return `<span style="color:${color}; font-weight:bold;" title="${tooltip}">B$${binanceUsdt2Fmt}</span>`;
}

async function loadKimchiPremium() {
  const listEl = qs("kimchiCompareList");
  const marketInput = qs("kimchiMarketInput");
  const exchangeRateEl = qs("kimchiExchangeRate");
  const rateLabel = qs("kimchiRateLabel");
  
  if (!listEl) return;
  
  listEl.innerHTML = '<div class="empty" style="color:#888; text-align:center;">조회 중...</div>';
  
  try {
    const markets = marketInput ? marketInput.value.trim() : "";
    
    // 환율: 입력값이 있으면 사용, 없으면 실시간 조회 (exchange_rate=0)
    let exchangeRateParam = 0;  // 0이면 서버에서 실시간 조회
    if (exchangeRateEl && exchangeRateEl.value) {
      const inputVal = parseFloat(exchangeRateEl.value);
      if (inputVal > 0) {
        exchangeRateParam = inputVal;
      }
    }
    
    const url = `/api/system/cross-compare?markets=${encodeURIComponent(markets)}${exchangeRateParam > 0 ? `&exchange_rate=${exchangeRateParam}` : ''}`;
    const res = await authFetch(url);
    const data = await res.json();
    
    // 서버에서 받은 실시간 환율로 UI 업데이트
    if (data.ok && data.exchange_rate) {
      state.exchangeRate = data.exchange_rate;
      if (exchangeRateEl && !exchangeRateEl.value) {
        exchangeRateEl.value = data.exchange_rate;
      }
      if (rateLabel) {
        rateLabel.innerHTML = `환율: <b style="color:#4caf50;">${data.exchange_rate.toLocaleString()}</b>`;
      }
    }
    
    if (!data.ok || !data.data || data.data.length === 0) {
      listEl.innerHTML = '<div class="empty" style="color:#666; text-align:center; padding:10px;">데이터 없음</div>';
      return;
    }
      ...
      <table style="width:100%; border-collapse:collapse; font-size:10px;">
        <thead>
          <tr style="border-bottom:1px solid #444; color:#888;">
            <th style="text-align:left; padding:4px;">코인</th>
            <th style="text-align:right; padding:4px;">업비트</th>
            <th style="text-align:right; padding:4px;">바이낸스</th>
            <th style="text-align:right; padding:4px;">김프</th>
          </tr>
        </thead>
        <tbody>
          ${data.data.map(item => {
            ...
            return `
              <tr style="border-bottom:1px solid #333;">
                ...
                <td style="text-align:right; padding:4px;">${formatPrice(item.bybit_usdt)}</td>
...
function bindKimchiControls() {
  ...
  // 환율 변경 시 자동 새로고침 & 전역 state 업데이트
  const exchangeRateEl = qs("kimchiExchangeRate");
  if (exchangeRateEl) {
    exchangeRateEl.addEventListener("change", () => {
      state.exchangeRate = parseFloat(exchangeRateEl.value) || 1450;
      loadKimchiPremium();
    });
  }
...
// ============================================================
// Rebound Opportunity - 전체 업비트 마켓에서 급락 후 반등 기회
// ============================================================
async function loadReboundOpportunity() {
  try {
    ...
    const res = await authFetch(API.marketRebound + `?top_n=5&min_volume_usdt=1000000&max_decline_pct=-3&timeframe=${timeframe}`);
    const data = await res.json();
...
// ============================================================
// RSI Ranking - 과매도 코인 순위
// ============================================================
async function loadRsiRanking() {
  try {
    const res = await authFetch(API.marketRsi + "?top_n=10&min_volume_usdt=1000000&rsi_max=40");
    const data = await res.json();
...
async function loadTechScore() {
  try {
    const res = await authFetch(API.marketTechScore + "?top_n=10&min_volume_usdt=1000000&min_score=50");
    const data = await res.json();
...
async function loadAllRankings() {
  try {
    const res = await authFetch(API.marketRankings + "?top_n=5&min_volume_usdt=1000000");
    const data = await res.json();
...
async function loadStrategyTab(tabName, apiUrl) {
  ...
  try {
    const res = await authFetch(apiUrl + "?top_n=20&min_volume_usdt=1000000");
...
function deriveManagedMarkets(sys) {
  ...
  // [2026-01-30] 포지션이 있는 마켓만 필터링
  // AUTOLOOP, PINGPONG, CONTRARIAN, SNIPER, GAZUA 표시
  managed = managed.filter(m => {
    const ctx = coord[m] || {};
    const pos = ctx.position || {};
    const qty = Number(pos.qty) || 0;
    // 포지션이 있거나, Reserved/SNIPER/GAZUA 전략인 경우 표시
    const hasPosition = qty > 0;
    const isReserved = (ctx.controls?.reserved === true) || (ctx._reserved === true);
    const strategy = String(ctx.controls?.strategy?.mode || ctx.strategy || '').toUpperCase();
    const alwaysShowStrategies = ['SNIPER', 'GAZUA'];
    return hasPosition || isReserved || alwaysShowStrategies.includes(strategy);
  });

  // fallback 제거: active_prices 전체를 가져오지 않음 (포지션 없는 코인 제외)
  return managed;
}
...
window.cancelQuickOrder = async function(quickId) {
  try {
    const res = await authFetch(`/api/trade/quick/${quickId}/cancel`, { method: "POST" });
    const data = await res.json();
    if (data.ok) {
...
async function executeQuickTrade(side) {
  ...
  try {
    const res = await authFetch("/api/trade/quick", {
      method: "POST",
...
// Update threshold label based on mode selection (% or $)
  ...
  estimateDebounceTimer = setTimeout(async () => {
    try {
      ...
      if (data.ok) {
        ...
        textEl.innerHTML = `
          <span style="color:#aaa;">${lookback}분 ${refLabel}:</span> <b>$${fmtPrice(data.reference_price)}</b> 
          → <span style="color:#fff;">진입가 $${fmtPrice(data.entry_price)}</span>
          <span style="color:${diffColor};">(현재가 대비 ${diffSign}${data.diff_pct.toFixed(2)}%)</span>
        `;
      } else {
...
async function fetchMarketSuggestions(query) {
  ...
  try {
    const res = await authFetch(`/api/trade/markets/suggest?query=${encodeURIComponent(query)}&limit=10`);
    const data = await res.json();
...
function renderGuardsPanel() {
  ...
  const GUARD_TOOLTIPS = {
    ...
    "g_min_order_usdt": "Min Order USDT: 최소 주문 금액 (원)",
    ...
    "g_exit_min_net_profit_usdt": "Exit Profit USDT: 최소 순수익 금액 (원)",
    ...
    "mg_exit_min_net_profit_usdt": "Exit USDT: 최소 순수익 금액",
...
function fmtKrShort(v) {
  ...
  const sign = n < 0 ? "-" : "+";
  if (abs >= 1e8) return `${sign}$${(Math.abs(n) / 1e8).toFixed(1)}억`;
  if (abs >= 1e4) return `${sign}$${(Math.abs(n) / 1e4).toFixed(0)}만`;
...
function applyReservedSettingsToUi(settings) {
  ...
  const aaCt = qs("aaContrarian");
  const aaSn = qs("aaSniper");
  ...
  if (aaCt) aaCt.checked = !!ap.auto_approve_contrarian;
  if (aaSn) aaSn.checked = !!ap.auto_approve_sniper;
}
...
function readReservedSettingsFromUi() {
  ...
  const aaCtEl = qs("aaContrarian");
  const aaSnEl = qs("aaSniper");
  ...
  const autoApproveContrarian = aaCtEl ? !!aaCtEl.checked : false;
  const autoApproveSniper = aaSnEl ? !!aaSnEl.checked : false;
  ...
  return {
    ...
    auto_approve_contrarian: autoApproveContrarian,
    auto_approve_sniper: autoApproveSniper,
  ...
  if (btn) {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        const data = await fetchJson(API.reservedRefresh(pp, al, ld, lt, gz, ct), { method: "POST" });
...
async function openSectorMapModal() {
  ...
  document.getElementById("saveSectorMap").addEventListener("click", async () => {
    ...
    try {
      ...
      const resp = await authFetch("/api/system/sector-map", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
