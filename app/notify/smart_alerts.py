...
# ------------------------------------------------------------
# 1. Consecutive loss warning (3-loss-streak alert)
# 2. Anomaly detection (loss much larger than average)
# 3. Automatic daily summary report dispatch
# ============================================================
...
class SmartAlertManager:
    ...
    def __init__(
        ...
        *,
        ...
        anomaly_threshold_pct: float = 200.0,  # loss >= 200% above average
        ...
        self.recent_losses: deque = deque(maxlen=100)  # recent loss history (for anomaly detection)
    ...
    def track_trade_result(
        ...
        if pnl_usdt < 0:
            ...
            # Alert when consecutive loss threshold is reached
    ...
    def _alert_loss_streak(self, streak: LossStreak):
        ...
        msg = (
            f"⚠️ Consecutive loss warning!\n\n"
            f"📉 Market: {streak.market}\n"
            f"🎯 Strategy: {streak.strategy}\n"
            f"🔴 Consecutive losses: {streak.consecutive_losses}\n"
            f"💸 Cumulative loss: {streak.total_loss_usdt:,.0f} USDT\n\n"
            f"You may want to review this strategy."
        )
    ...
    def check_anomaly(self, market: str, loss_usdt: float) -> Optional[AnomalyDetection]:
        """Anomaly detection (loss much larger than average)"""
        if loss_usdt >= 0:  # ignore if not a loss
            return None

        loss_abs = abs(loss_usdt)

        # Compute recent loss average
        if len(self.recent_losses) < 5:  # need at least 5 samples
            return None
        ...
        # Flag anomaly when threshold is exceeded
    ...
    def _alert_anomaly(self, anomaly: AnomalyDetection):
        """Anomalous trade alert"""
        msg = (
            f"🚨 Anomalous trade detected!\n\n"
            f"📊 Market: {anomaly.market}\n"
            f"💥 Loss: {anomaly.loss_usdt:,.0f} USDT\n"
            f"📈 Average loss: {anomaly.avg_loss_usdt:,.0f} USDT\n"
            f"⚡ Deviation: +{anomaly.deviation_pct:.0f}%\n\n"
            f"A loss much larger than usual has occurred."
        )
    ...
    def generate_daily_report(
        ...
        for market, agg in aggs.items():
            ...
            # Per-strategy aggregation (roughly inferred from market name)
            strategy = "UNKNOWN"
            if "PINGPONG" in market.upper():
                strategy = "PINGPONG"
            elif "AUTOLOOP" in market.upper():
                strategy = "AUTOLOOP"
            # ... (should really be pulled from the ledger)
    ...
    def send_daily_report(self, report: DailyReport):
        ...
        msg = (
            f"{emoji} Daily trade report ({report.date})\n"
            f"{'=' * 30}\n\n"
            f"📊 Total trades: {report.total_trades}\n"
            f"✅ Wins: {report.wins}\n"
            f"❌ Losses: {report.losses}\n"
            f"🎯 Win rate: {report.win_rate:.1f}%\n\n"
            f"💰 Total PnL: {report.total_pnl_usdt:+,.0f} USDT ({status})\n\n"
        )

        if report.best_strategy:
            msg += f"🏆 Best strategy: {report.best_strategy}\n"
        if report.worst_strategy:
            msg += f"⚠️ Worst strategy: {report.worst_strategy}\n\n"
        ...
        if report.alerts_count > 0:
            msg += f"⚠️ Warnings today: {report.alerts_count}\n"
    ...
    def check_and_send_daily_report(self, ledger_records: List[Dict]):
        """Check whether to auto-send the daily report"""
        now = datetime.now()
        ...
        # Check whether the configured time has been reached
