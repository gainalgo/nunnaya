...
class HarpoonConfig:
    ...
    server_side_tpsl: bool = True       # Server-side TP/SL required (safety: stops stay on the exchange even if the bot is down)
    ...
    paper_mode: bool = True               # ★ 2026-04-23 owner directive: paper concept removed — ON=live trading, OFF=fully halted
    # Stage 0-1: B11 regime_lock integration (FOCUS focus_mgr._get_btc_regime_lock_reason)
    respect_b11_regime_lock: bool = False   # BULL → block SHORT, BEAR → block LONG
    ...
    respect_focus_adx_slope: bool = False   # If FOCUS J v2 is on, apply the same criteria
    # Stage 0-4: Morning Guard auto-standby window
    respect_morning_guard: bool = False     # Auto-standby during 06:50~morning_guard_end
    # Stage 0-5: share coin_loss_cap
    respect_coin_loss_cap: bool = False     # Share FOCUS's 24h cumulative loss cap
    # Stage 0-6: HARPOON-only Fast-Reject (forensic 04-14 peak 0% pattern)
    fast_reject_v2_enabled: bool = False    # 60s after entry, peak 0% + loss → immediate cut
    ...
    respect_focus_coin_lock: bool = False    # HARPOON skips coins FOCUS holds
    respect_focus_direction_lock: bool = False  # Forbid scalps opposite to FOCUS direction
    ...
    def _save_config(self):
        ...
        try:
            ...
            try:
                ...
                maybe_snapshot(CONFIG_PATH, data["config"])
            except Exception:
                pass
