...
class SpotGazuaConfig:
    enabled: bool = False              # ★ ON by default (2026-06-17 owner: "everything ON, except paper"). paper=True so 0 real orders.
    paper: bool = True               # ★ LIVE (real orders) — 2026-06-21 owner "all 3 Live" (GAZUA recovery method armed). Reverses old keep-paper stance. Per-server paper=True revert possible via UI/runtime.
    ...
    use_pct_tp: bool = False            # default fixed % (rotation)
    ...
    longhold_enabled: bool = False      # ★ ON by default (2026-06-17 owner) — observe behavior in paper
    ...
    dca_enabled: bool = False            # core of the recovery method. 0=OFF (simple SL/longhold only)
    ...
    dca_stabilize_gate_enabled: bool = False   # short-term stabilization check before DCA. 0=OFF (old unconditional averaging)
    ...
    contrarian_regime_gate: bool = False         # True=don't enter in an uptrend (BTC UP), neutral/down only. False=always
    ...
    gap_check_breakout_exempt: bool = False       # [2026-06-20] exempt gap on new-high breakout — a breakout isn't top-chasing (breakout coins are always right near their own high, so the anchor alone can't clear it). Pump tops are caught by headroom/late-stage/micro
    gap_check_min_pct: float = 0.3               # minimum required gap %
    gap_check_atr_adaptive: bool = False          # volatility-adaptive (required gap = max(min, ATR%×mult))
    ...
    momentum_deriv_require_both: bool = False      # True=only when both RSI+MACD are against (conservative)
    ...
    entry_expectation_enabled: bool = False
    ...
    entry_expectation_bypass_guard_score: bool = False  # exempt guard_score-pass from EE gate (★ON · skip RR gate for score-pass · futures mirror. OFF=old behavior)
    ...
    adx_entry_breakout_exempt: bool = False   # even at low ADX, pass if the last closed bar broke the last N-bar high (box-recovery entry)
    ...
    phase3_context_bonus_enabled: bool = False  # Phase 3 time-of-day (±4) + coin (+2) bonus ON/OFF
    ...
    btc_trend_conv_bonus_enabled: bool = False
    btc_trend_conv_bonus: float = 20.0       # [2026-05-17 100pt ×10] 2→20
    multi_be_lock_atr_adaptive_enabled: bool = False   # True=ATR-multiple mode / False=fixed-% mode (uses stage_pct above)
    ...
    be_lock_smart_rsi_check: bool = False       # ① RSI in profit direction → hold off firing
    be_lock_smart_candle_check: bool = False    # ② last N bars consecutively in profit direction → hold off firing
    ...
    smart_manual_entry_enabled: bool = False            # ON/OFF (when OFF, hide L⏳/S⏳ buttons)
    ...
    be_stall_intelligent_enabled: bool = False  # [2026-05-15 owner] momentum-linked (MACD/RSI/BB) intelligent — live implementation
    ...
    min_tp_distance_enabled: bool = False
    ...
    microtiming_5m_phase_k_exempt: bool = False       # exempt Phase K (regime transition) entries — prioritize catching market transitions
    micro_1m_candle_check: bool = False             # ① check last 1M bar direction
    micro_1m_candle_trend_exempt_adx: float = 0.0  # ADX above this = strong trend → exempt 1M bar direction check (0=disabled)
    micro_1m_volume_check: bool = False             # ② check 1M volume decline
    micro_1m_rsi_check: bool = False                # ③ check 1M RSI extremes
    ...
    overextension_enabled: bool = False                 # live ON (owner 2026-06-07 "live not paper")
    ...
    blowoff_chase_only: bool = False                    # True = penalize same-direction (chase) only / fade (opposite) exempt
    headroom_penalty_enabled: bool = False              # [2026-06-10 owner "you input it and turn it on"] fleet default ON
    ...
    awaken_require_day_align: bool = False              # only Day (coin D1) in-trend qualifies to hold (exclude against/undecided)
    ...
    override_slot_enabled: bool = False
    ...
    override_require_btc_trend_match: bool = False  # BTC trend match required (Phase 2B)
    ...
    expectation_progress_exit_enabled: bool = False    # progress-based exit (replaces LHT time-cuts)
    ...
    expectation_progress_neg_cut_enabled: bool = False
    ...
    entry_expectation_gate_enabled: bool = False       # ★ #1: block on RR/risk/reward threshold
    ...
    regime_counter_strong_cap_enabled: bool = False   # cap conviction in the against direction on STRONG against ON
    regime_counter_strong_cap: float = 50.0          # cap value (below scanner/guard threshold → no entry + score clearly drops)
    coin_decouple_enabled: bool = False          # per-coin decoupling SHORT release ON/OFF (2026-06-12 owner "default ON")
    ...
    macro_recovering_require_di_adx: bool = False     # dead-cat defense: LONG bonus only with +DI>-DI flip + ADX≥threshold together
    ...
    entry_flip_require_alignment: bool = False         # ★ #2: block if FLIP direction is against both H1+30M
    entry_auto_flip_enabled: bool = False              # ★ auto-FLIP permanently disabled after the ICP incident
    gap_check_atr_adaptive_enabled: bool = False    # volatility-adaptive ON (block top-chasing in a swinging market)
    ...
    entry_volatility_gate_enabled: bool = False         # verify reach feasibility just before entry (default ON)
    ...
    btc_emergency_pause_enabled: bool = False          # BTC sharp-move detection ON/OFF
    ...
    btc_emergency_aggressive_entry: bool = False       # accelerate trend-direction entry on empty slots ON/OFF
    ...
    day_direction_enabled: bool = False
    ...
    h4_pa_snapshot_enabled: bool = False
    h4_pa_snapshot_hours_kst: str = "1,5,9,13,17,21"  # CSV — KST hours at 4-hour intervals (H4 candle close times)
    morning_shield_enabled: bool = False        # 06:00 KST: tighten SL on profitable positions
    ...
    event_shield_enabled: bool = False          # event window: block new entry + tighten held SL
    ...
    event_shield_auto_fetch: bool = False       # [2026-06-08 owner] auto-fetch ForexFactory USD High impact (union with manual input)
    ...
    dual_direction_observe: bool = False        # Phase 1 observe ON (no entry change · collect data only)
    gate_ledger_enabled: bool = False           # B: per-gate pass/reject tally ('why was it silent' control panel). Observe-only · doesn't touch entry · 100% local (no cross-server Tick). 2026-06-21 owner control panel — default ON (paper-observe principle, instrument for diagnosing 0 entries).
    ...
    coin_repeat_brake_enabled: bool = False     # brake on repeated entry into the same coin
    ...
    sl_decay_enabled: bool = False             # [2026-04-25 Long Hold System] ★ must be OFF (opposite of Long Hold)
    ...
    post_trade_pause_loss_sliding_enabled: bool = False   # ON by default (owner decision)
    ...
    direction_exhaustion_enabled: bool = False
    ...
    profit_exit_block_enabled: bool = False
    ...
    profit_exit_block_block_opposite: bool = False     # opposite direction allowed by default (preserve FLIP opportunity)
    ...
    s3_gate_enabled: bool = False                       # [2026-04-25 default promotion] standardize Fee-Aware Gate (False→True)
    ...
    reentry_cooldown_v2_enabled: bool = False           # ★default ON (2026-06-21 AXS rotation fix) — block re-entry into the just-closed coin for N min. Wired in _scan_and_maybe_enter.
    ...
    imminent_flip_enabled: bool = False
    imminent_flip_ema_gap_pct: float = 0.3            # BTC |EMA20-EMA50|/price*100 threshold
    imminent_flip_use_30m: bool = False                # 30M secondary signal (False = H1 alone)
    ...
    momentum_deriv_guard_enabled: bool = False
    ...
    mtf_momentum_align_enabled: bool = False
    ...
    mtf_momentum_align_use_macd: bool = False          # include MACD too (False = RSI only)
    mtf_momentum_align_rsi_slope_thr: float = 0.5     # RSI Δ sign threshold (ignore small changes)
    cfid_enabled: bool = False
    ...
    cfid_bypass_momentum_deriv: bool = False           # enable momentum_deriv BLOCK bypass
    cfid_bypass_mtf_align: bool = False                # enable mtf_momentum_align BLOCK bypass
    ...
    cfid_leading_bypass_microtiming: bool = False      # bypass 5m gate
    cfid_leading_bypass_bb_regime: bool = False        # bypass BB_REGIME peak block
    ...
    pattern_leading_bypass_microtiming: bool = False
    pattern_leading_bypass_bb_regime: bool = False
    ...
    phase6_combo_e_enabled: bool = False               # E: intuition bonus ON/OFF
    ...
    phase6_combo_f_enabled: bool = False               # F: bar-flow bonus ON/OFF
    combo_f_dedupe_enabled: bool = False               # remove combo_f direction double-count (F1 MTF·F2 M5) (2026-06-12 owner "default ON")
    ...
    charge_exit_enabled: bool = False                  # auto-close on score recovery ON/OFF
    ...
    bb_trend_bypass_require_di: bool = False           # ★ ② direction confirmed — for SHORT, wall-ride only with -DI>+DI (real downtrend). Filters out 'about to bounce (reversal)'. If DI unreadable, ① only.
    ...
    macro_exit_strong_coin_exempt: bool = False       # per-coin-strength exemption ON (don't cut a profitable position even on macro-against)
    macro_exit_exempt_min_roe: float = 0.0           # exempt if profit exceeds this price-ROE% (default 0=any profit exempts)
    coin_state_machine_enabled: bool = False           # classify + log + stamp into entry dict
    ...
    tight_trail_after_be_enabled: bool = False
    ...
    tight_trail_atr_adaptive_enabled: bool = False
    ...
    tf_round_tpsl_enabled: bool = False               # explicit force ON (incl. LIVE). default decided by auto_paper
    ...
    tf_round_hold_enabled: bool = False               # hold (short-cut off) — accompanies mode ON
    ...
    frame_guard_option_b_enabled: bool = False           # explicit force ON (incl. LIVE)
    ...
    frame_guard_cooldown_enabled: bool = False
    frame_guard_cooldown_sec: float = 90.0           # silent-skip period for the same (market, direction) after a block
    h4_pulse_only_enabled: bool = False              # explicit force ON (incl. LIVE)
    ...
    preclose_body_dir_required: bool = False          # use body direction + close position (top/bottom 30%) condition
    ...
    preclose_topup_require_candle_dir: bool = False   # top-up only if the last closed H4 bar is the entry direction
    preclose_topup_grace_min: float = 60.0           # top-up-allowed window after H4 close (min) — expires past it (keep half)
    anchor_fasttrack_enabled: bool = False           # explicit force ON (incl. LIVE)
    anchor_fasttrack_max_proximity: float = 0.33     # within ⅓ of TP1 distance = fast-track fires
    pa_completion_enabled: bool = False              # explicit force ON (incl. LIVE)
    ...
    final_30m15m_check_enabled: bool = False          # block entry if both 30M+15M are against
    ...
    final_5m_simple_check_enabled: bool = False       # check 5M RSI/MACD/BB agreement with entry direction
    final_5m_simple_min_score: int = 2               # pass if N+ of 3 agree (max 3)
    final_5m_bb_trend_bypass_enabled: bool = False
    final_d1_alignment_check_enabled: bool = False    # block entry on D1 against-direction
    final_align_regime_override_enabled: bool = False   # when macro is certain, final alignment gate prefers macro direction (default ON)
    ...
    exit_5m_emergency_enabled: bool = False          # explicit force ON (incl. LIVE)
    ...
    guard_dir_dedupe_enabled: bool = False            # remove direction double-count (Frame/Trend/AltBTC/BTC-align) on the guard side (2026-06-12 owner "default ON")
    ...
    flow_reversal_signal_auto_paper: bool = False       # auto ON in paper mode
    ...
    alt_btc_alignment_enabled: bool = False             # always evaluate (safe)
    ...
    day_box_guard_enabled: bool = False              # explicit force ON (incl. LIVE)
    ...
    h1_pa_pulse_enabled: bool = False                # explicit force ON (incl. LIVE)
    ...
    h1_pa_pulse_require_day_dir: bool = False         # force H4 day_direction alignment (NEUTRAL = passes)
    ...
    coin_reentry_penalty_enabled: bool = False
    ...
    fast_reject_enabled: bool = False           # [2026-04-25 Long Hold System] 5~15-min cut OFF (recovery patience)
    ...
    btc_regime_enabled: bool = False         # [2026-04-25 default promotion] standardize BTC against-direction penalty (False→True)
    ...
    pair_block_enabled: bool = False           # ★ OFF by default — when active, block the opposite direction
    ...
    coin_profit_lockin_enabled: bool = False   # ★ OFF by default
    ...
    coin_profit_lockin_require_be: bool = False     # activate only after BE lock (avoid H4-strategy conflict)
    pa_weight_enabled: bool = False             # ON by default (owner decision)
    ...
    phase4_rsi_enabled: bool = False
    phase4_mtf_matrix_enabled: bool = False
    phase4_change_rate_enabled: bool = False
    phase4_sr_position_enabled: bool = False
    phase4_volume_pattern_enabled: bool = False
...
class SpotGazuaManager:
    ...
    def _save_state(self) -> None:
        try:
            ...
            try:
                from app.core.config_history import maybe_snapshot
                maybe_snapshot(self.state_path, data["config"])
            except Exception:
                pass
        except Exception as exc:
